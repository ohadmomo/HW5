...
DeliveryCompany::~DeliveryCompany() {
	unsigned int i = 0;
	DeliveryVehicle** head_of_list = &(pdvl->front());
	for (i = 0; i < pdvl->size(); i++)
	{
		if (head_of_list[i] != NULL)
			delete head_of_list[i];
	}
	delete[] pdvl;
}
...
bool DeliveryCompany::receiveParcel(Parcel* parcel) {
	...
	if (next_to_deliver == NULL) 
		next_to_deliver = head_of_queue[0];
	else 
	...
	{
		if (car_queue[i]->addParcel(parcel) == false)continue;//check if there is a case for parcel with sam id
		else {
			if (pdvl->size() - 1 == i)
				next_to_deliver = car_queue[0];
			else
				next_to_deliver = car_queue[i + 1];
			return true;
		}
		for (i = 0; i < next_to_deliver_index; i++) {
			if (car_queue[i]->addParcel(parcel) == false)continue;
			else {
				next_to_deliver = car_queue[i+1];	
				return true;
			}
...
int DeliveryCompany::getIndex(DeliveryVehicle* vehicle) const {
	...
	DeliveryVehicle** head_of_queue = (DeliveryVehicle**)(pdvl->front());
	for (i = 0; i < pdvl->size(); i++) {
		if (head_of_queue[i] == vehicle) return i;
	}
...
bool DeliveryCompany::performDeliveryDay() {
	...
	for (i = 0; i < pdvl->size(); i++) {
		daily_revenue += head_of_queue[i]->performDeliveryDay(&num_of_daily_deliveries);
